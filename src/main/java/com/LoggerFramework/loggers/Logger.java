...
import com.LoggerFramework.utils.LoggerUtils;
...
public class Logger implements ILogger {
    ...
    public void fatal(Object object,String text){
        if(!LoggerUtils.getInstance().isLoggingAllowed(LoggerEnum.FATAL_LOG_LEVEL.name())) return;
        logProcessor.process(LoggerEnum.FATAL_LOG_LEVEL,object.getClass().getName(),text);
    }

    @Override
    public void error(Object object,String text){
        if(!LoggerUtils.getInstance().isLoggingAllowed(LoggerEnum.ERROR_LOG_LEVEL.name())) return;
        logProcessor.process(LoggerEnum.ERROR_LOG_LEVEL,object.getClass().getName(),text);
    }

    @Override
    public void warn(Object object,String text){
        if(!LoggerUtils.getInstance().isLoggingAllowed(LoggerEnum.WARN_LOG_LEVEL.name())) return;
    ...
    public void info(Object object,String text){
        if(!LoggerUtils.getInstance().isLoggingAllowed(LoggerEnum.INFO_LOG_LEVEL.name())) return;
        logProcessor.process(LoggerEnum.INFO_LOG_LEVEL,object.getClass().getName(),text);
    }

    @Override
    public void debug(Object object,String text){
        if(!LoggerUtils.getInstance().isLoggingAllowed(LoggerEnum.DEBUG_LOG_LEVEL.name())) return;
